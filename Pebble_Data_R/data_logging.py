...
#context = zmq.Context()
#ssocket = context.socket(zmq.PUSH)
#ssocket.connect("tcp://127.0.0.1:5000")
class DataLoggingService(EventSourceMixin):
    ...
    def download(self, session_id):
        ...
        timeout_count = 0
        while True:
            logger.info("Looping again. Time: {}".format(datetime.datetime.now()))
            try:
                result = queue.get(timeout=5).data
                timeout_count = 0
            except TimeoutError:
                logger.info("Got timeout error Time: {}".format(datetime.datetime.now()))
                timeout_count += 1
                if timeout_count >= 2:
                    break
                else:
            ...
            if isinstance(result, DataLoggingDespoolSendData):
                if result.session_id != session_id:
                    ...
                    # Send data out to zmq socket
                    ssocket.send(msgpack.packb((result.data)))
                    if data is None:
                        data = result.data
                    else:
                        data += result.data
